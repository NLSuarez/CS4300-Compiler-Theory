...
char* C_KEYWORD_ARRAY[KEYWORD_COUNT] = {
    "cin",
    "cout",
    "else",
    "endl",
...
// Useful for tracking control structures (IF/ELSE and WHILEs) during eval phase
struct control_node* control_list_head;
struct control_node* control_list_tail;
...
struct strlit_node* appendToStrList(STRLIT_LIST* head, char* str, int eval_state) {
    ...
    if (* head == NULL) {* head = (struct strlit_node* ) malloc(sizeof(struct strlit_node));
        (* head)->str = strdup(str);
        (* head)->loc = 0;
        (* head)->next = NULL;
        ptr =* head;
...
struct var_node* appendToGlobalVarList(char* var_id, int kind) {
    ...
    if (ptr == NULL) {
        ...
        ptr->kind = kind;
        ptr->isGlobal = 1;
	} else { // Search for existing variable.
        while (ptr->next != NULL) {
...
struct var_node* appendToFuncVars(struct symbol_node *
    var) {
...
void popControl() {
    struct control_node* ptr = control_list_tail;

	while(control_list_tail->next != ptr)
		control_list_tail = control_list_tail->next;

    free(ptr);
}

struct control_node* pushControl(int kind) {
    struct control_node* ptr = malloc(sizeof(struct control_node));
	
    ptr->VMQ_bool_list = ptr->VMQ_true_list = ptr->VMQ_false_list = NULL;
    ptr->VMQ_bool_line_count = ptr->VMQ_true_line_count = ptr->VMQ_false_line_count = 0;
    ptr->kind = kind;

    if (control_list_head != NULL)
		control_list_tail->next = ptr;
	else
		control_list_head = control_list_tail = ptr;

	ptr->next = NULL;

    return control_list_tail = ptr;
}
...
int main(int argc, char* * argv) {
    ...
    // Add global int constants for true and false.
    ...
    if (list_ptr != NULL) {
        if (int_list_ptr != NULL)
            list_ptr->loc = int_list_ptr->loc + 2;

        if (global_var_list_ptr != NULL)
            list_ptr->loc = global_var_list_ptr->loc + 2;
    ...
    char* str_ptr = filename + strlen(filename) - 3;* str_ptr = 'q';
    str_ptr++;* str_ptr = '\0';
    ...
    int line_counter = 1;
    char* line_str = malloc(8);
    while (func_list_ptr != NULL) {
        fputs(func_list_ptr->VMQ_stack_frame->str, VMQ_file);
        for (int i = 0; i < 20 - strlen(func_list_ptr->VMQ_stack_frame->str); ++i) fputs(" ", VMQ_file);
        sprintf(line_str, ";\t%d", line_counter++);
        fputs(line_str, VMQ_file);
        fputs("\n", VMQ_file);

        list_ptr = func_list_ptr->VMQ_list;
        while (list_ptr != NULL) {
            fputs(list_ptr->str, VMQ_file);
            for (int i = 0; i < 20 - strlen(list_ptr->str); ++i) fputs(" ", VMQ_file);
            sprintf(line_str, ";\t%d", line_counter++);
            fputs(line_str, VMQ_file);
            fputs("\n", VMQ_file);
            list_ptr = list_ptr->next;
...
unsigned int func_toss_return = 0;
unsigned int true_in_progress = 0;
unsigned int false_in_progress = 0;
unsigned int nested_counter = 0;

FUNC_LIST current_func = NULL;

STRLIT_LIST* VMQ_statement_list;

// Helper function for eval.  Appends WHILE/IF related VMQ
// statements to current_func->VMQ_list in the proper order.
void appendControlStatements(struct control_node* cn)
{
	VMQ_statement_list = &(current_func->VMQ_list);
	STRLIT_LIST ptr = cn->VMQ_bool_list;
	
	for(int i = 0; ptr && i < cn->VMQ_bool_line_count; ++i)
	{
		appendToStrList(VMQ_statement_list, ptr->str, 1);
		ptr = ptr->next;
	}

	ptr = cn->VMQ_true_list;
	
	
	for(int i = 0; ptr && i < cn->VMQ_true_line_count; ++i)
	{
		while(strcmp(ptr->str, "signal") == 0)
		{
			appendControlStatements(cn->next);
			ptr = ptr->next;
		}
	...
	ptr = cn->VMQ_false_list;
	
	
	for(int i = 0; ptr && i < cn->VMQ_false_line_count; ++i)
	{
		while(strcmp(ptr->str, "signal") == 0)
		{
			appendControlStatements(cn->next);
...
struct ast* eval(struct ast* a) {
        ...
        case STREAMIN:
            ...
            struct var_node *
                var = ((struct symref* ) a->r)->vn;

            if (var->isParam)
                sprintf(VMQ_add_stmt, "p /%d",
                    var->loc);
            ...
            appendToStrList(VMQ_statement_list, VMQ_add_stmt, 1);
            appendToStrList(VMQ_statement_list, "c 0 -1", 1);
            appendToStrList(VMQ_statement_list, "^ 2", 1);
        ...
        case RETURN:
            if (strcmp(current_func->func->symbol, "main") == 0) break;
        ...
        case FUNC:
            lnode = a->l; // lnode will be the ID of the function
            ...
            if (func_toss_return)
                sprintf(VMQ_add_stmt, "c 0 %d", ptr->VMQ_line_start);
            else
        ...
        case ASSIGNOP:
            ...
            */
            if (rnode && rnode->nodetype == ID) {
                ...
                while (expr_max_temp_vars != 0) { /* "s tmp tmp tmp" will set tmp to 0. */
        ...
        case '+':
            ...
            if (lnode && lnode->nodetype != INT_LITERAL) {
                eval(a->l); /*temp_vars--;*/
			}
            if (rnode && rnode->nodetype != INT_LITERAL) { /*temp_vars++;*/
                eval(a->r);
			}
        ...
        case ('+' + '='):
            lnode = a->l; // lnode is always a variable
        ...
        case STREAMOUT:
            ...
            eval(a->l); // Takes us to bottom of the output tree (first output statement to execute).
            ...
            if (rnode != NULL)
                ...
                case STR_LITERAL:
                    ;
                    char* VMQ_push_stmt;
                    VMQ_push_stmt = (char* ) malloc(20);
                    STRLIT_LIST ptr = str_list_head;
                    char* str = strdup(((struct stringval* ) rnode)->str->str);
                    while (ptr != NULL) {
                        if (strcmp(ptr->str, str) == 0) break;
                        else ptr = ptr->next;
					}
                    sprintf(VMQ_push_stmt, "p #%d", ptr->loc);
                    appendToStrList(VMQ_statement_list, VMQ_push_stmt, 1);
                    current_func->VMQ_line_count++;
                    appendToStrList(VMQ_statement_list, "c 0 -11", 1);
                    current_func->VMQ_line_count++;
                    appendToStrList(VMQ_statement_list, "^ 2", 1);
                    current_func->VMQ_line_count++;
                    break;
                ...
                case INT_LITERAL:
                    ;
                    char* VMQ_intlit_stmt = malloc(20);
                    sprintf(VMQ_intlit_stmt, "p #%d", ((struct intval* ) rnode)->number->loc);
                    appendToStrList(VMQ_statement_list, VMQ_intlit_stmt, 1);
                    current_func->VMQ_line_count++;
                    appendToStrList(VMQ_statement_list, "c 0 -9", 1);
                    current_func->VMQ_line_count++;
                    appendToStrList(VMQ_statement_list, "^ 2", 1);
                    current_func->VMQ_line_count++;
                    current_func->VMQ_line_count += 3;
            ...
            // statement
        case ('s' + 't' + 'm' + 't'):
            if (a->l->nodetype == FUNC) func_toss_return = 1;
            else func_toss_return = 0;
            ...
            // statements
        case ('s' + 't' + 'm' + 't' + 's'):
            if (a->l != NULL) eval(a->l);
            if (a->r != NULL) eval(a->r);
            break;

            // block
        case ('b' + 'l' + 'k'):
            eval(a->l); // Variable definitions
            eval(a->r); // Statements
            break;
        ...
        case ('p' + 'r' + 'o' + 'g'):
            eval(a->r);
            appendToStrList(VMQ_statement_list, "h", 1);
            current_func->VMQ_line_count++;
            break;

/*
	The problem:	We can make separate lists for each WHILE/IF structure, but we need a way to submit them into the
					final VMQ_list in the proper order.  We cannot do this by just appending each list from each WHILE/IF
					since they must be interleaved in a certain way.  This makes nested WHILE/IFs difficult to manage.

	The solution:	During the body of a while loop, or during the body of an if/else, it is possible to encounter another
	(hopefully)		WHILE/IF.  When this happens in the eval function, we will have another control_node generated and the
					proper statements will be recorded.  However, due to some of the intricacies of this approach (primarily
			        due to needing to count lines for jump statements), some statements must be added in at a later time.
					One way to ensure ordering would be to insert a "signal" node into the current true or false list being 
					appended to when a new WHILE or IF is encountered.  When printing all of the stmts to the final VMQ_list,
					we can compare each node's string to the signal message.  When we encounter a signal message, we can then
					switch to the deeper control_node and append as usual.  Anytime a signal node is encountered, we switch
					to a deeper control_node.  After all of the statements for a control_node are appended, we pop the node
					and continue where we left off from the previous control_node.
*/
			
		case WHILE:

			;STRLIT_LIST* prev_VMQ_list = NULL;
			
			if(true_in_progress || false_in_progress)
			{
				nested_counter++;
				
				if(true_in_progress && !false_in_progress)
				{
					// Need to insert signal into outer control structure's true_list
					appendToStrList(VMQ_statement_list, "signal", 1);
					prev_VMQ_list = VMQ_statement_list;
				}
				else if(!true_in_progress && false_in_progress)
				{
					// Need to insert signal into outer control structure's false_list
					appendToStrList(VMQ_statement_list, "signal", 1);
					prev_VMQ_list = VMQ_statement_list;
				}
			}

            struct control_node* this_node = pushControl(WHILE); // Generate a new control struct for while loop 

            lnode = a->l; // Some sort of boolean expression
            rnode = a->r; // Statements to execute if true.

			// Save current line count prior to eval'ing conditional.
            int lines = current_func->VMQ_line_start + current_func->VMQ_line_count;
            VMQ_statement_list = &(this_node->VMQ_bool_list);
			
            eval(lnode);

// We put in the final conditional check late, so we need to store
// the location of the true/false result for use later.
            int bool_temp_loc = 2*(temp_vars + current_func->var_count);

// Update the line count for boolean conditional statements.
            this_node->VMQ_bool_line_count += current_func->VMQ_line_start + current_func->VMQ_line_count - lines;

// Save current line count prior to eval'ing true statements
            lines = current_func->VMQ_line_start + current_func->VMQ_line_count;
            VMQ_statement_list = &(this_node->VMQ_true_list);

            temp_vars = 0;
            expr_max_temp_vars = 0;
...
// Update the line count for true statements.
            this_node->VMQ_true_line_count += current_func->VMQ_line_start + current_func->VMQ_line_count - lines;

            VMQ_statement_list = &(this_node->VMQ_bool_list);

// Put in final check for conditional true/false
            sprintf(VMQ_add_stmt, "e /-%d 000 %d", bool_temp_loc, current_func->VMQ_line_start + current_func->VMQ_line_count + nested_counter + 3);
            appendToStrList(VMQ_statement_list, VMQ_add_stmt, 1);
			this_node->VMQ_bool_line_count++;
			current_func->VMQ_line_count++;
 
			VMQ_statement_list = &(this_node->VMQ_true_list);
            sprintf(VMQ_add_stmt, "j %d", current_func->VMQ_line_start + current_func->VMQ_line_count - this_node->VMQ_true_line_count - this_node->VMQ_bool_line_count + nested_counter + 1);
            appendToStrList(VMQ_statement_list, VMQ_add_stmt, 1);
			this_node->VMQ_true_line_count++;
            current_func->VMQ_line_count++;

            temp_vars = 0;
            expr_max_temp_vars = 0;

			if(this_node == control_list_head)
				appendControlStatements(this_node);

			if(prev_VMQ_list != NULL) VMQ_statement_list = prev_VMQ_list;

			if(nested_counter > 0) nested_counter--;
        ...
        case IF:
            ...
            rnode = ((struct flow*) a)->el; //Where to go if the cond is false
			
			this_node = pushControl(IF);
			...
			bool_temp_loc = 2*(temp_vars + current_func->var_count);
            ...
            VMQ_statement_list = &(this_node->VMQ_false_list);
			...
			this_node->VMQ_false_line_count += current_func->VMQ_line_start + current_func->VMQ_line_count - lines;

			VMQ_statement_list = &(this_node->VMQ_true_list);

			sprintf(VMQ_add_stmt, "j %d", current_func->VMQ_line_start + current_func->VMQ_line_count + nested_counter + 3);
			appendToStrList(VMQ_statement_list, VMQ_add_stmt, 1);
			this_node->VMQ_true_line_count++;
			current_func->VMQ_line_count++;
...
// left/right child may be expressions, must be eval'd first.
            eval(lnode);
            eval(rnode);
            ...
            if (lnode->nodetype == ID) {
                if (((struct symref* ) lnode)->vn->isParam)
                    l_op_ref = "@/";
                else
                    l_op_ref = "/-";
			} else if (lnode->nodetype == INT_LITERAL)
                l_op_ref = "";
            else
                l_op_ref = "/-";
            ...
            if (lnode->nodetype == ID)
                l_loc = ((struct symref* ) lnode)->vn->loc;
            else if (lnode->nodetype == INT_LITERAL)
                l_loc = ((struct intval* ) lnode)->number->loc;
            else
                l_loc = 2*(temp_vars + current_func->var_count);

            if (rnode->nodetype == ID)
                r_loc = ((struct symref* ) rnode)->vn->loc;
            else if (rnode->nodetype == INT_LITERAL)
                r_loc = ((struct intval* ) rnode)->number->loc;
            else
                r_loc = 2*(temp_vars + current_func->var_count + 1);

// Need to store true/false value in a new temp_var.
            if (++temp_vars > func_max_temp_vars) func_max_temp_vars = temp_vars;
            if (temp_vars > expr_max_temp_vars) expr_max_temp_vars = temp_vars;

// Less than condition line
            sprintf(VMQ_add_stmt, "l %s%d %s%d %d", l_op_ref, l_loc, r_op_ref, r_loc, current_func->VMQ_line_start + current_func->VMQ_line_count + nested_counter + 4);
            appendToStrList(VMQ_statement_list, VMQ_add_stmt, 1);
            current_func->VMQ_line_count++;

// If false, store 0 in temp var
            sprintf(VMQ_add_stmt, "i 000 /-%d", 2*(temp_vars + current_func->var_count));
            appendToStrList(VMQ_statement_list, VMQ_add_stmt, 1);
            current_func->VMQ_line_count++;

// Jump statement to skip the true stuff below.
            sprintf(VMQ_add_stmt, "j %d", current_func->VMQ_line_start + current_func->VMQ_line_count + nested_counter + 3);
            appendToStrList(VMQ_statement_list, VMQ_add_stmt, 1);
            current_func->VMQ_line_count++;

// If true, store 1 in temp var
            sprintf(VMQ_add_stmt, "i 002 /-%d", 2*(temp_vars + current_func->var_count));
            appendToStrList(VMQ_statement_list, VMQ_add_stmt, 1);
            current_func->VMQ_line_count++;
...
// Greater than condition line
            sprintf(VMQ_add_stmt, "g %s%d %s%d %d", l_op_ref, l_loc, r_op_ref, r_loc, current_func->VMQ_line_start + current_func->VMQ_line_count + nested_counter + 4);
            appendToStrList(VMQ_statement_list, VMQ_add_stmt, 1);
            current_func->VMQ_line_count++;
...
// left/right child may be expressions, must be eval'd first.
            ...
            if (rnode->nodetype == ID)
                ...
                r_loc = 2*(temp_vars + current_func->var_count);

// Need to store true/false value in a new temp_var.
            if (++temp_vars > func_max_temp_vars) func_max_temp_vars = temp_vars;
            if (temp_vars > expr_max_temp_vars) expr_max_temp_vars = temp_vars;

// Equal to condition line
            sprintf(VMQ_add_stmt, "e %s%d %s%d %d", l_op_ref, l_loc, r_op_ref, r_loc, current_func->VMQ_line_start + current_func->VMQ_line_count + nested_counter + 4);
            appendToStrList(VMQ_statement_list, VMQ_add_stmt, 1);
            current_func->VMQ_line_count++;
