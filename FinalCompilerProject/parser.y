...
/*	Tokens go here; Names and Literal values	*/
	...
	%token <s> CIN COUT ELSE ENDL FLOAT IF INT RETURN WHILE 
	%token <fn> FUNC
	%token EOL
	...
	%type <str> output_statement expression term factor simple_expression variable program function_definitions	function_head block identifier_list variable_definitions type arguments parameter_list parameters statements statement compound_statement bool_expression expressions input_statement expression_list bool_term bool_factor
	%type <a> literal
	%start program
%%
